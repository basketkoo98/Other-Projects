...
public class MovingRotorTest {
    ...
    public void testAtNotch() {
        MovingRotor p = new MovingRotor(
                "mr", new Permutation(NAVALA.get("I"), UPPER), "BC");
        p.set(1);
        assertTrue(p.atNotch());
        p.set(2);
        assertTrue(p.atNotch());
        p.set(3);
        assertFalse(p.atNotch());
    }
    @Test
    public void testAdvance() {
        MovingRotor p2 = new MovingRotor(
                "mr2", new Permutation(NAVALA.get("I"), UPPER), "BC");
        p2.set(14);
        p2.advance();
        assertEquals(15, p2.setting());
        p2.advance();
        assertEquals(16, p2.setting());
        p2.set(25);
        p2.advance();
        assertEquals(0, p2.setting());
        p2.set(0);
        p2.advance();
        assertEquals(1, p2.setting());
    }
}
